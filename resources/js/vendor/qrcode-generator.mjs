...
//---------------------------------------------------------------------
//
// Vendorizado direto (não via npm) — `node_modules/` nesta máquina de dev
// ficou com dono `root` (criado por uma sessão anterior via Docker) e este
// ambiente não tem `sudo`/Docker disponíveis pra rodar `npm install` e
// regravar o `package-lock.json` corretamente. Arquivo copiado sem
// modificação de github.com/kazuhikoarase/qrcode-generator (js/dist/qrcode.mjs)
// — se um dia `node_modules` for corrigido (`sudo chown -R $USER node_modules`),
// dá pra trocar por `npm install qrcode-generator` sem mudar a API usada
// aqui (Shared/Components/QrCodeSvg.vue usa só `qrcode()`, `.addData()`,
// `.make()`, `.createSvgTag()`).
//
//---------------------------------------------------------------------
